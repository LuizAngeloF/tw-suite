...
## Fase 11 — v1.8.0: relógio ao vivo, saque por recurso, gráfico de histórico, fila de construção filtrada

Pedido do usuário depois de testar o v1.7.0 ao vivo: contagem de "2 a caminho" sem tempo nenhum, filas de construção/recrutamento só atualizando na sincronização (não em tempo real), saque só mostrando o total (não por recurso), fila de construção do dashboard mostrando passos já concluídos (ex.: "Bosque → nv.1" com o Bosque já em nível 10), e um pedido de gráfico de histórico. Perguntei (96% de confiança) sobre o escopo do gráfico antes de começar — respostas: todas as 3 métricas (saque por recurso, vitórias/derrotas, perdas dos dois lados), retenção de 7-14 dias com agregação diária, gráfico por conta no dossiê.

| Mudança | Como funciona | Status |
|---|---|---|
| Relógio ao vivo (construção/recrutamento/comandos) | `buildLiveSnapshot()` agora converte o texto de contagem regressiva ("0:11:31") num horário-alvo absoluto (`etaAtMs`/`arrivesAtMs`, `parseCountdownToMs`) na hora da sincronização. Um `setInterval` de 1s (painel do jogo e dashboard, cada um com o seu) recalcula o texto a partir desse horário-alvo — sem nenhuma requisição nova — e a sincronização seguinte (~15-25s no jogo, ~20s no dashboard) só corrige o valor pro real | **Testado no navegador**: injetei horários-alvo fictícios no dashboard e confirmei a contagem diminuindo sozinha (0:03:18 → 0:03:04 depois de ~14s) sem nova sincronização |
| Estimativa de ida+volta dos comandos enviados | A tela só mostra a hora de CHEGADA, nunca quando foi enviado. Aproximamos "enviado" pela primeira vez que o ciclo (`live-status`) viu aquele comando (`storage:'live-status:cmdFirstSeen'`, por id do comando) — volta estimada = 2×chegada − primeira-vez-visto. Só fica disponível a partir do 2º ciclo vendo o mesmo comando; exibido com "~" pra deixar claro que é estimativa | **UNVERIFIED contra o jogo real** — a lógica foi testada isolada no dashboard com dado fictício, mas depende de observar um comando de verdade em dois ciclos seguidos pra confirmar a precisão |
| Saque por recurso (não só total) | `battle-reports` já capturava `loot: {wood, stone, iron}` desde o v1.7.0, só não estava sendo mostrado — trocado `saque 200` por `🪵200 🧱150 ⛏50` tanto no painel do jogo quanto no dashboard | Reaproveita dado já **VERIFIED** (Fase 10), só mudança de exibição |
| Fila de construção do dashboard oculta passos já concluídos | Antes mostrava sempre o plano configurado inteiro, sem comparar contra o nível real. Agora `buildLiveSnapshot()` expõe `buildingLevels` (direto de `game_data.village.buildings`, já carregado em toda página, sem requisição extra) e o editor de fila (`queueEditorHTML`) filtra passos onde `nível atual >= nível do passo`, com uma nota "N passo(s) já concluído(s), ocultos daqui" | **Testado no navegador** com dado fictício — 4 de 15 passos corretamente ocultos, nota exibida |
| Gráfico de histórico (novo cartão "Histórico") | `battle-reports` agora também agrega por dia (`storage:'battle-reports:daily'`, chave `AAAA-MM-DD` local, teto de 14 dias) — vitórias/derrotas, saque por recurso, perdas dos dois lados. Dashboard renderiza 3 mini-gráficos de barra em SVG puro (sem biblioteca): saque empilhado (madeira/argila/ferro), vitórias×derrotas agrupado, perdas agrupado — com legenda e tooltip nativo (`<title>` do SVG) por barra. Cores: as 3 do saque são as do slot categórico 1/2/3 já validados no `references/palette.md` da skill `dataviz` (`#3987e5`/`#d95926`/`#199e70`); vitória/derrota e perdas reaproveitam `--jade`/`--rose`, já usados no resto do dashboard pro mesmo sentido (bom/ruim) | **Testado no navegador** com 7 dias de dado fictício — as 3 séries renderizam certo, com legenda e rótulo de dia |
...
## Fase 13 — v1.10.0: correções e polimento depois do primeiro uso real completo

Feedback do usuário depois de usar o v1.9.0 com a conta real de verdade (primeira vez vendo tudo junto: relatórios, relógio, construção, coleta). Confirmou estar no v1.9.0 antes de reportar — então os itens abaixo são bugs/pedidos reais, não desatualização.

| Item | Causa / decisão | Correção |
|---|---|---|
| Relógio do Recrutamento não andava (comandos enviados sim) | `etaAtMs` depende do script ter processado aquele ciclo com o campo novo; se o dado sincronizado no navegador ainda não tinha isso, o dashboard caía pro texto `remaining` congelado, sem alternativa | Novo `effectiveEtaAtMs()` no dashboard: se `etaAtMs` não veio, deriva um horário-alvo equivalente a partir de `remaining` (texto "H:MM:SS") + `buildQueueAt`/`trainQueueAt` (quando aquele snapshot foi capturado) — o relógio ao vivo passa a funcionar mesmo com dado sincronizado sem o campo novo. **Testado no navegador**: contagem decrescendo sozinha só com `remaining`+`trainQueueAt`, sem `etaAtMs` |
| "Perdi 2" sem dizer qual tropa | `summarize()` só guardava o total (`lostAtt`), descartava o detalhe por unidade que `getAttackReportDetail` já lia desde a Fase 10 | Novo `lostAttUnits`/`lostDefUnits` no resumo (objeto por tropa, só as com perda > 0). Exibido como "2 Lanceiro" em vez de "2" |
| Lista de relatórios "feia", pedido de tabela melhor | — | Nova `battleTableHTML()`: tabela de verdade (ícone de resultado, alvo, 3 colunas de saque por recurso, perdas) em vez da lista de uma linha por relatório |
| "Histórico" sempre vazio mesmo com relatórios na lista | Bug real: `battleDaily` só recebia relatórios NOVOS (`newOnes`) depois do v1.8.0 — quem já tinha relatórios processados antes disso (marcados em `seen`) nunca teria esses dias agregados, porque a lista `seen` existe pra evitar reprocessar | Backfill de uma vez: se `daily` está vazio mas `log` já tem itens, reconstrói a agregação inteira a partir do log existente, antes de processar novos relatórios |
| "Retorno de \<aldeia\>" aparecendo em "Comandos enviados" | O widget do jogo (`#commands_outgoings`) lista o retorno das próprias tropas pra casa dentro do mesmo lugar de onde vêm os ataques enviados — não existe um campo separado pra distinguir, só o texto do rótulo | Dashboard detecta pelo prefixo "Retorno" no rótulo (única pista disponível) e move esses itens pra "Chegando", com ícone próprio (↩) distinto do ⚠ de ataque inimigo |
| Pedido de botão pra recolher cada seção (Construindo/Recrutando/Coletando/Comandos/Chegando/Relatórios) | O cartão "Ao vivo" é redesenhado a cada 1s pelo relógio — uma classe CSS sozinha se perderia a cada tick | Novo `state.liveOpen` (Set, mesmo padrão do `state.openFine` já usado nos "Ajustes finos"), lido a cada render — sobrevive ao tick de 1s. Testado clicando e conferindo `getBoundingClientRect().height === 0` na seção recolhida |
