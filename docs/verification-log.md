...
**Fase 1 considerada verificada ponta a ponta** (descoberta de alvo → preencher formulário → resolver alvo → enviar → tela de confirmação → botão final identificado). Ainda não testamos um envio com `autoConfirm` ligado de fato clicando "Enviar ataque" sozinho — o usuário testou manualmente até a confirmação e parou aí por escolha própria.
